                ...
                <div class="col-12">
                  <div class="card top-selling overflow-auto">
                    ...
                    <div class="card-body pb-0">
                      <h5 class="card-title">{{property.title}} Tenants <span>| Tenants under M-Keja properties</span></h5>
                      <p class="card-text">
                        <div class="row">
                      ...
                      <table id="AllLandlordsTable" class="table table-borderless">
                        ...
                        <tbody v-else>
                        <tr v-for="tenancy in tenants" :key="tenancy.id">
                            ...
                            <span class="badge bg-info">
                                {{ tenancy.unit?.unit_number ?? 'Unassigned' }}
                            </span>
                            </td>

                            <!-- Phone -->
                            <td>{{ tenancy.tenant?.phone ?? 'N/A' }}</td>

                            <!-- Tenancy Status -->
                            <td>
                            <span v-if="tenancy.status === 'active'" class="badge bg-success">
                                <i class="bi bi-check-circle me-1"></i> Active
                            </span>

                            <span v-else-if="tenancy.status === 'pending'" class="badge bg-warning text-dark">
                            ...
                            <div class="btn-group" role="group">
                                ...
                                <a @click="viewTenant(tenancy.tenant)" class="dropdown-item">
                                    <i class="ri-eye-fill me-2"></i> View Tenant
                                </a>

                                <a @click="viewUnit(tenancy.unit)" class="dropdown-item">
                                    <i class="ri-home-4-line me-2"></i> View Unit
                                </a>

                                <a @click="endTenancy(tenancy.id)" class="dropdown-item text-danger">
                                    <i class="ri-close-circle-line me-2"></i> End Tenancy
                                </a>
                                </div>
                            </div>
                            </td>
   ...
   import DefaultProfile from '@/assets/img/default-profile.png'
    ...
    export default {
      ...
      methods: {
        ...
        const propertyId = this.$route.params.id; // Get property ID from route

        axios.get(`/api/properties/${propertyId}/tenants`) // Call API for tenants of this property
            .then((response) => {
            this.tenants = response.data.tenants; // Only tenants for this property
            this.property = response.data.property; // Optional: property info
            this.properties = response.data.properties; // Optional: property info
            console.log(response);
